...
// Canteen schema uses conditional validation based on the tab
...
export const ServiceRequest: React.FC<ServiceRequestProps> = ({ serviceId, onBack }) => {
  ...
  const [formData, setFormData] = useState<Record<string, any>>({
    // Defaults to avoid uncontrolled input warnings
    canteenTab: 'new_emp',
    ...
    return (
      <div className="max-w-2xl mx-auto text-center py-12 px-4" aria-live="polite">
        ...
        <p className="text-slate-500 mb-8 max-w-md mx-auto leading-relaxed">
          Your request has been successfully queued. Your reference ID is <span className="font-mono font-medium text-slate-900 bg-slate-100 px-1.5 py-0.5 rounded">REQ-{Math.floor(Math.random() * 10000)}</span>.
        </p>
...
const FormFields: React.FC<FormFieldsProps> = ({ serviceId, formData, errors, onChange }) => {
  ...
  switch (serviceId) {
    case ServiceType.SAFETY:
      return (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
          ...
          <Select 
            label="Type of Guard Required" 
            value={formData.guardType || ''}
            onChange={h('guardType')}
            error={errors.guardType}
            options={[
              { value: 'armed', label: 'Armed Guard' },
              { value: 'unarmed', label: 'Unarmed Guard' },
        ...
        <div className="space-y-8">
          <fieldset>
            ...
            <div className="flex p-1 bg-slate-100 rounded-lg w-full sm:w-fit">
              <label 
                className={`
                  flex-1 sm:flex-none text-center px-6 py-2.5 text-sm font-medium rounded-md transition-all duration-200 cursor-pointer 
                  focus-within:ring-2 focus-within:ring-indigo-500 focus-within:ring-offset-2 focus-within:ring-offset-slate-100
                  ${formData.keyServiceType === 'courier' 
              ...
              >
                <input 
                  ...
                  value="courier" 
                  checked={formData.keyServiceType === 'courier'}
                  onChange={() => onChange('keyServiceType', 'courier')}
                  className="sr-only"
                />
                Courier Dispatch
              </label>
              <label 
                className={`
                  flex-1 sm:flex-none text-center px-6 py-2.5 text-sm font-medium rounded-md transition-all duration-200 cursor-pointer 
                  focus-within:ring-2 focus-within:ring-indigo-500 focus-within:ring-offset-2 focus-within:ring-offset-slate-100
                  ${formData.keyServiceType === 'document' 
                    ? 'bg-white text-indigo-600 shadow-sm ring-1 ring-black/5' 
                    : 'text-slate-600 hover:text-slate-900 hover:bg-slate-200/50'
                  }
                `}
              >
                <input 
                  ...
                  name="key_service_type" 
                  value="document" 
                  checked={formData.keyServiceType === 'document'}
                  onChange={() => onChange('keyServiceType', 'document')}
                  className="sr-only"
                />
                Document Handling
              </label>
      ...
      return (
        <div className="space-y-6">
          ...
          <TextArea 
            ...
            value={formData.venueReqs || ''}
            onChange={h('venueReqs')}
            error={errors.venueReqs}
            required 
      ...
      return (
        <div className="space-y-8">
           <div className="border-b border-slate-200">
            <div role="tablist" aria-label="Canteen Request Type" className="-mb-px flex space-x-8">
              <button
                ...
                role="tab"
                id="tab-new-emp"
                aria-selected={formData.canteenTab === 'new_emp'}
                aria-controls="panel-new-emp"
                onClick={() => onChange('canteenTab', 'new_emp')}
                className={`
                  pb-4 px-1 border-b-2 font-medium text-sm whitespace-nowrap transition-colors duration-200
                  focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:rounded
                  ${formData.canteenTab === 'new_emp' 
              ...
              <button
                ...
                role="tab"
                id="tab-guest"
                aria-selected={formData.canteenTab === 'guest'}
                aria-controls="panel-guest"
                onClick={() => onChange('canteenTab', 'guest')}
                className={`
                  pb-4 px-1 border-b-2 font-medium text-sm whitespace-nowrap transition-colors duration-200
                  focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-500 focus-visible:rounded
                  ${formData.canteenTab === 'guest' 
                    ? 'border-indigo-600 text-indigo-600' 
                    : 'border-transparent text-slate-500 hover:text-slate-700 hover:border-slate-300'
                  }
                `}
          ...
          <div 
            role="tabpanel" 
            id="panel-new-emp" 
            aria-labelledby="tab-new-emp"
            hidden={formData.canteenTab !== 'new_emp'}
            className={formData.canteenTab === 'new_emp' ? "space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300 focus:outline-none" : "hidden"}
            tabIndex={0}
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Input 
                ...
                required={formData.canteenTab === 'new_emp'}
              />
              <Input 
                label="Full Name" 
            ...
            <Select 
              ...
              onChange={h('dietaryPref')}
              error={errors.dietaryPref}
              required={formData.canteenTab === 'new_emp'}
              options={[
          ...
          <div 
            ...
            hidden={formData.canteenTab !== 'guest'}
            className={formData.canteenTab === 'guest' ? "space-y-6 animate-in fade-in slide-in-from-bottom-2 duration-300 focus:outline-none" : "hidden"}
            tabIndex={0}
          >
            <Input 
              ...
              onChange={h('hostId')}
              error={errors.hostId}
              required={formData.canteenTab === 'guest'}
            />
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Input 
      ...
      return (
        <div className="space-y-6">
          <Select 
            ...
            value={formData.transportReqType || ''}
            onChange={h('transportReqType')}
            error={errors.transportReqType}
            options={[
              { value: 'pickup', label: 'Pick-up' },
              { value: 'drop', label: 'Drop-off' },
              { value: 'adhoc', label: 'Ad-hoc (Hourly)' },
            ]}
            required
          />
          <Input 
        ...
        <div className="space-y-8">
           <fieldset>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <label 
                className={`
                  relative flex items-start p-4 rounded-xl border-2 transition-all duration-200 cursor-pointer
                  hover:bg-slate-50 focus-within:ring-2 focus-within:ring-indigo-500 focus-within:ring-offset-2
                  ${formData.commType === 'sim' 
              ...
              >
                <div className="flex h-5 items-center">
                  <input 
                    ...
                    checked={formData.commType === 'sim'} 
                    onChange={() => onChange('commType', 'sim')} 
                  />
                </div>
                <div className="ml-3 text-sm">
                  <span className={`block font-medium ${formData.commType === 'sim' ? 'text-indigo-900' : 'text-slate-900'}`}>New SIM Card</span>
                  <span className={`block mt-1 ${formData.commType === 'sim' ? 'text-indigo-700' : 'text-slate-500'}`}>Voice & SMS enabled for corporate use</span>
                </div>
                <div className={`absolute top-4 right-4 ${formData.commType === 'sim' ? 'text-indigo-600' : 'text-slate-300'}`}>
                  <Signal size={20} />
                </div>
              </label>
              ...
              >
                <div className="flex h-5 items-center">
                  <input 
                    ...
                    value="bundle" 
                    className="h-4 w-4 border-slate-300 text-indigo-600 focus:ring-indigo-600" 
                    checked={formData.commType === 'bundle'} 
                    onChange={() => onChange('commType', 'bundle')} 
                  />
                </div>
                <div className="ml-3 text-sm">
                  <span className={`block font-medium ${formData.commType === 'bundle' ? 'text-indigo-900' : 'text-slate-900'}`}>Data Bundle</span>
                  <span className={`block mt-1 ${formData.commType === 'bundle' ? 'text-indigo-700' : 'text-slate-500'}`}>Add-on Internet Pack for existing line</span>
                </div>
                <div className={`absolute top-4 right-4 ${formData.commType === 'bundle' ? 'text-indigo-600' : 'text-slate-300'}`}>
                  <FileText size={20} />
                </div>
              </label>
            </div>
      ...
      return (
        <div className="space-y-6">
          <fieldset>
              ...
              {['General Cleaner', 'Labor Support', 'Tea / Pantry Boy'].map((item) => (
                <label 
                  ...
                  className={`
                    ...
                    hover:bg-slate-50
                    ${(formData.housekeepingServices || []).includes(item)
                ...
                >
                  <input
                    ...
                    checked={(formData.housekeepingServices || []).includes(item)}
            ...
            {errors.housekeepingServices && (
              <p className="mt-2 text-sm text-red-600">{errors.housekeepingServices}</p>
            )}
          </fieldset>
          <Input 
            label="Location / Floor / Room" 
            placeholder="e.g. 3rd Floor, Conference Room B" 
            value={formData.location || ''}
            onChange={h('location')}
            error={errors.location}
            required 
          ...
          <TextArea 
            ...
            placeholder="e.g. Deep cleaning required for..." 
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <Select 
              ...
              options={[
                ...
                { value: 'plumbing', label: 'Plumbing / Washroom' },
                { value: 'carpentry', label: 'Carpentry / Furniture' },
                { value: 'construction', label: 'Minor Construction' },
              ]}
              required
            />
            <Input 
              label="Location" 
              placeholder="Specific area requiring maintenance" 
              value={formData.location || ''}
              onChange={h('location')}
              error={errors.location}
              required 
    ...
    default:
      return <p className="text-slate-500 italic">Form details not available.</p>;
  }
};